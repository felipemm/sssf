      ...
      <div v-else class="runs">
        <div v-for="r in data.running" :key="r.adwId" class="run">
          ...
          <span class="run-actions">
            <button class="icon" title="Stop — finalizes the session + in-flight phases as 'fail' (stopped by the engineer)" aria-label="Stop session" :disabled="pending.has(`stop:${r.adwId}`)" @click="onStop(r.project, r.adwId)">
              <Square :size="13" />
            </button>
            <button class="icon" title="Restart — reuses the adw_id + request and attaches to the same branch" aria-label="Restart session" :disabled="pending.has(`restart:${r.adwId}`)" @click="onRestart(r.project, r.adwId)">
              <RotateCw :size="13" />
            </button>
          </span>
        </div>
