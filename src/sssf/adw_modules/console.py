...
class Console:
    ...
    def session_started(self, adw_id: str, engineer: str) -> None:
        self._emit(
            f"[bold cyan]adw_id:[/bold cyan] [bold]{escape(adw_id)}[/bold]"
    ...
    def session_finished(self, ok: bool, tokens: int, cost: float, db_path: str) -> None:
        ...
        plain = (
            f"session {self.adw_id} {'success' if ok else 'fail'} · "
    ...
    def phase_started(self, phase: Phase) -> None:
        ...
        line = (
            f"[bold {color}]▶ {phase.seq:02d} {escape(p.name)}[/bold {color}]"
    ...
    def agent_started(self, name: str, model: str, session_id: str) -> None:
        self._emit(
            f"  [magenta]▸[/magenta] {escape(name)} [dim]{escape(model)}[/dim]"
    ...
    def retry(self, name: str, attempt: int, limit: int, reason: str) -> None:
        self._emit(
            f"  [yellow]⟳[/yellow] {escape(name)} retry {attempt}/{limit} "
    ...
    def gate_result(self, name: str, report) -> None:
        ...
        self._emit(
            f"  {mark} gate [dim]{escape(name)}[/dim] [dim]{summary}[/dim]",
        ...
        for check in report.checks:
            ...
            self._emit(
                f"    [{style}]{'·' if check.ok else '✗'} {escape(_clip(check.item))}"
