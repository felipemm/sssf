...
class DocumentOutput(EnvelopeBase):
    ...
    document_path: str = ""  # the doc in the repo, e.g. adws/kb/<adw_id>_<slug>.md
...
class SandboxConfig(BaseModel):
    ...
    The old `review` block (launch the app after the run and keep the
    container up) is DROPPED (ADR-0004): the runner executes work and exits;
    the deploy flow's QA surface is the workbench (workbench.py), a separate
    disposable container from the `dev` branch."""
