...
def resolve_base(ref: str) -> BaseRef:
    ...
    if git_helper.short_sha(base.commit) != git_helper.short_sha("HEAD"):
        base.reason = (
            f"HEAD is ahead of {base.label} — diffing every commit since, plus the working tree"
        )
        ...
        base.reason = f"HEAD is on {base.label} with a clean tree — falling back to the last commit"
    else:
...
def as_envelope(changes: ChangeSet, notes: str = "") -> ChangesOutput:
    ...
    return ChangesOutput(
        status="success",
        summary=(
            f"{total} file(s) changed since {changes.base.label} "
