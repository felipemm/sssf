...
class PhaseHandle:
    ...
    def log(self, **payload) -> None:
        self.run.tracer.event(
            EventRecord(
                adw_id=self.run.adw_id,
                phase_id=self.phase.phase_id,
                type="log",
                name=self.phase.params.name,
                payload=payload,
            )
        )
        self.run.console.note(", ".join(f"{k}: {v}" for k, v in payload.items()))
        if self.phase.params.kind == "engineer" and "input" in payload:  # noqa: SIM102 — nested on purpose, see below
...
class Run:
    def __init__(self, cfg, adw_id: str, tracer, engineer: str):
        ...
        # bookkeeping, the flow chains' flags). Typed here so code phases can
        # set them without mypy attr-defined errors; the chain layer owns the
        # values.
        ...
        self._skip_exploration = False  # adw_plan: --skip-exploration
        self._deploy_yes = False  # adw_deploy: --yes (signoff gate)
    ...
    def phase(self, params: PhaseParams):
        self._seq += 1
        phase = Phase(
            phase_id=f"{self.adw_id}_{self._seq:02d}_{params.name}",
            adw_id=self.adw_id,
            seq=self._seq,
            params=params,
            status="running",
