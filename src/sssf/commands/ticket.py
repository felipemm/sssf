...
def run(ticket_id: str, project: str | None = None, no_sandbox: bool = False) -> int:
    ...
    row = conn.execute(
        ...
        " prompt_file FROM tickets WHERE id=?",
        (ticket_id,),
    ).fetchone()
    if row is None:
        conn.close()
        ...
        return 1
    tid, title, description, status, provider, external_id, source_url, prompt_file = row
    if status == "running":
        conn.close()
    ...
    if sandboxed:
        ...
        if prompt_file and (root / prompt_file).exists():
            ...
            conn.commit()
            conn.close()
            print(f"sssf ticket: run spawned for {tid} — prompt adws/prompts/{rel_prompt.name} (existing prompt_file)")
            return 0
