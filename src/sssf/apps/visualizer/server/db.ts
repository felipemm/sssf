...
const DEFAULT_DB_RELATIVE = "adws/data/sssf.db";
 ...
 * state after a run finishes) cannot be read by a readonly connection, because
 * SQLite must create the shm to read WAL. Probe and fall back to a read-write
 * open, which may create it; we never write through this connection.
 */
...
/**
 * Resolve the db path: --db arg wins, then SSSF_DB, then <cwd>/adws/data/sssf.db.
...
export class SssfDb {
   ...
   * `adws/data/sssf.db`), so deriving it as a sibling of the db file keeps
  ...
  constructor(path: string, db?: Database) {
    ...
    // A caller-supplied connection is the multi-project case: the registry
    // already opened it read-only and caches it; we just bind the queries.
    this.db = db ?? openReadonly(path);
