    ...
    <div class="d-grid">
        ...
        >
          ...
          <template v-else-if="promptsState === 'ready'">
            ...
            <div v-for="panel in promptPanels" :key="panel.id" class="prompt-panel">
              <button class="prompt-head" :aria-expanded="openPanels.has(panel.id)" :aria-label="`Toggle prompt ${panel.title}`" @click="togglePanel(panel.id)">
        ...
        >
          ...
          <div v-for="g in phaseGates" :key="g.id" class="gate" :class="g.passed ? 'pass' : 'fail'">
            <template v-if="gateChecks(g)">
              <button class="gate-line gate-toggle" :aria-expanded="openGates.has(g.id)" :aria-label="`Toggle gate ${g.gate}`" @click="toggleGate(g.id)">
      ...
      <div class="d-col">
        ...
        <div v-for="e in phaseEvents" :key="e.event_id" class="event">
          <button class="event-row" :class="{ open: expanded.has(e.event_id) }" :aria-expanded="expanded.has(e.event_id)" aria-label="Toggle event details" @click="toggle(e)">
